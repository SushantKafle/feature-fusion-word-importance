...
def create_feedforward(input_tensor, input_size, output_size, fn_initializer, activation, scope):
	with tf.variable_scope(scope):
		weights = tf.get_variable("W_", dtype=tf.float32, shape=(input_size, output_size)) #initializer=fn_initializer((input_size, output_size)))
		bias = tf.get_variable("b_", dtype=tf.float32, shape=(output_size, )) # initializer=fn_initializer((output_size,)))
